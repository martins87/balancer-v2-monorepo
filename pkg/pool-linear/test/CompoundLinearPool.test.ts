...
describe('CompoundLinearPool', function () {
  ...
  describe('getWrappedTokenRate', () => {
    it('returns the expected value', async () => {
      ...
      const compoundStartingRate = 2e16;
      await mockLendingPool.setRate(bn(2e16));
      expect(await pool.getWrappedTokenRate()).to.be.eq(compoundStartingRate);

      // We now double the reserve's normalised income to change the exchange rate to 2:1
      // await mockLendingPool.setRate(bn(2e27));
