...
describe('CompoundLinearPoolFactory', function () {
  ...
  before('setup signers', async () => {
    ...
    console.log("0 - setup signers")
  });

  sharedBeforeEach('deploy factory & tokens', async () => {
    vault = await Vault.create();
    const queries = await deploy('v2-standalone-utils/BalancerQueries', { args: [vault.address] });
    // console.log('deploy factory & tokens:', [vault.address, vault.getFeesProvider().address, queries.address])
    ...
    const wrappedTokenInstance = await deploy('MockCToken', { args: ['cDAI', 'cDAI', 18, mainToken.address] });
  ...
  describe('constructor arguments', () => {
    ...
    sharedBeforeEach('create pool', async () => {
      ...
      console.log("passou de createPool()")
    });

    it('sets the vault', async () => {
      console.log("2 - sets the vault")
